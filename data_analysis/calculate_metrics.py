...
def calculate_metrics_per_shot_llm(shot_path, shot='shot_0', exp_name=experiment_name, rank_size='size_3'):
    ...
    ranked_files = [f for f in ranked_folder if f.endswith('.csv') and 'ranked' in f and 'ground_truth' not in f]
    ...
    with open(metrics_path, 'w', newline='') as f_metrics:
        ...
        for file in ranked_files:
            ...
            # if 'ListNet' in exp_name:
            #     GT_score = np.array(ground_truth_df["GT_score"])
            # else:
            #     GT_score = np.array(ranked_df[score_column])
    ...
    # calculte skew and safe the results


def CalculateResultMetrics(meta_exp='meta-llama/Meta-Llama-3-8B-Instruct'):
