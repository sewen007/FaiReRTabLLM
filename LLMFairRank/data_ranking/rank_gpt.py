...
def get_prefix_prompt(query, hits, rank_end, num, Item, model_type, prompt_id):
    ...
    if model_type == "meta-llama/Meta-Llama-3-8B-Instruct" or model_type == "gemini-1.5-pro":
        combined_content += "\n\nOUTPUT FORMAT\n" + (
    ...
    if experiment_name == 'LAW':
        return [{'role': 'system',
                 'content': f"You are the ranking officer for a Law school and your goal is to rank {Item}s by "
        ...
        return [{'role': 'system',
                ...
                {'role': 'user',
                 'content': f"I will provide a list of {num} {Item}s, each described by a sequential"
                            f" index (e.g., [1]), an ID, sex and a score representing performance. The list is "
                            f"already ranked by official time scores, from fastest to slowest, and you are to re-rank "
                            f"to incorporate fairness {instruct_prompt}"}]
        ...
        return [{'role': 'system',
                 'content': f"You are the loan officer for a bank and your goal is to rank loan applicants by "
                            f"predicted likelihood of default, while ensuring fairness."},
                {'role': 'user',
                 'content': f"I will provide a list of {num} {Item}s, each described by a sequential"
                            f" index (e.g., [1]), sex and the applicant's income. The list is already ranked by"
                            f" applicant income, and you are to re-rank to incorporate fairness {instruct_prompt}"}]
...
def create_permutation_instruction(df, item=None, rank_start=0, rank_end=50, item_type='applicant', prompt_id=1,
    ...
    messages = get_prefix_prompt(query, hits, rank_end, num, item_type, model_type, prompt_id)  # Get prefix prompt messages
...
def run_llm_deep(messages, api_key="deep_api_key", model_name="deepseek-chat"):
