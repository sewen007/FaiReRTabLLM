...
def writeRanked(writefile, dict):
    ...
    if experiment_name == 'LAW':
        fields = fields = ['doc_id', 'Gender', 'UGPA', 'LSAT', 'predictions', 'GT_score', 'Gender']
    else:
        fields = ['doc_id', 'Gender', 'predictions', 'GT_score', 'Gender']
...
def infer_with_detconstsort(file, rank_test=False, post_process=False, test_data=False, p_value=None):
    ...
    if rank_test:
        ...
        if test_data:
            ...
            write_path = f'./Datasets/{experiment_name}/Train/size_{match_size.group(1)}/Fair_Reranking/shot_' + match.group(1) + '/'
            if not os.path.exists(write_path):
                os.makedirs(write_path)
    else:
        write_path = './Datasets/' + experiment_name + '/Train/Fair/'
    ...
    for i in range(k_max):
        if result_scores[i][1] not in ranked_dict.keys():
            ...
            if experiment_name == 'LAW':
                UGPA = gt_data.loc[gt_data['doc_id'] == result_scores[i][1], 'UGPA'].iloc[0]
                LSAT = gt_data.loc[gt_data['doc_id'] == result_scores[i][1], 'LSAT'].iloc[0]

                ranked_dict[result_scores[i][1]] = [result_scores[i][1], gt_gender, UGPA, LSAT, result_scores[i][0],
                                                    gt_score,
                                                    result_genders[i]]
            else:
